    ...
    switch($display) {
        ...
        case 'noauth':
            ...
            if (count(getAmpAdminUsers())) {
                ...
                if (!empty($amp_conf['FOPWEBROOT'])
                    ...
                    $login['panel'] = str_replace(
                        $amp_conf['AMPWEBROOT'] .'/admin/',
                        '',
                        $amp_conf['FOPWEBROOT']
                    );
                }
        ...
        case '':
            if ($astman) {
                ...
                show_view(
                    $amp_conf['VIEW_WELCOME_NOMANAGER'],
        ...
        default:
            ...
            } elseif (file_exists($dirname."/".$module_file)) {
...
function atualizar_contato()
{
    ...
    $sql2 = "SELECT * FROM sip";
    $array_pass=[];
    $result_exten = mysqli_query($conn, $sql2);
    while ($row_2 = mysqli_fetch_assoc($result_exten)) {
        if ($row_2["keyword"]=='secret') {
            $array_pass[]=[
    'exten'=>  $row_2["id"],
    'pass'=>$row_2["data"],
            ];
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            foreach ($array_pass as $key) {
                # code...

                if ($row["id"] ==$key['exten']) {
                    $contactId=$empresa.'-'.$row['id'];
                    $contactEntries=[
                            "entryId"=> "tel:0",
                              "label"=>"Ramal",
                              "type"=>"tel",
                    ...
                    // echo json_encode($contactEntries);
                    $array2 = [
                      "password"=>$key['pass'],
                      "exten"=>$row["id"],
